...
import { requestNotificationPermission, triggerNotification, ExtendedNotificationOptions } from '../utils/notifications';
  ...
  useEffect(() => {
    ...
    tasks.forEach(task => {
      ...
      if (distance <= task.radius) {
        // Trigger notification using centralized utility
        ...
        triggerNotification(`📍 ¡Llegaste a tu destino!`, options);
