...
export const useGeofencing = (tasks: Task[]) => {
  ...
  const [triggeredTasks, setTriggeredTasks] = useState<Set<string>>(() => {
    try {
      const stored = localStorage.getItem('triggeredTasks');
      if (stored) {
        return new Set(JSON.parse(stored));
      }
    } catch (e) {
      console.error("Error loading triggered tasks from localStorage", e);
    }
    return new Set();
  });
  ...
  useEffect(() => {
    try {
      ...
      console.error("Error saving triggered tasks to localStorage", e);
    }
  }, [triggeredTasks]);
