...
describe('TaskCard', () => {
  ...
  it('renders task location when provided', () => {
    ...
    render(<TaskCard task={taskWithLocation} onToggle={mockOnToggle} onDeleteClick={mockOnDeleteClick} onEdit={mockOnEdit} />);
  ...
  it('calculates and shows distance when user location is provided', () => {
    ...
    render(<TaskCard task={taskWithLocation} userLat={40.4168} userLng={-3.7038} onToggle={mockOnToggle} onDeleteClick={mockOnDeleteClick} onEdit={mockOnEdit} />);
  ...
  it('calls onToggle when circle button is clicked', () => {
    ...
    const { container } = render(<TaskCard task={baseTask} onToggle={mockOnToggle} onDeleteClick={mockOnDeleteClick} onEdit={mockOnEdit} />);
  ...
  it('renders completed task with line-through styling', () => {
    ...
    const { container } = render(
      <TaskCard task={completedTask} onToggle={mockOnToggle} onDeleteClick={mockOnDeleteClick} onEdit={mockOnEdit} />
    );
  ...
  it('renders due date badge when dueDate is provided', () => {
    ...
    render(<TaskCard task={taskWithDate} onToggle={mockOnToggle} onDeleteClick={mockOnDeleteClick} onEdit={mockOnEdit} />);
    // Should render a date string (format depends on locale, but some date text should appear)
    // The calendar badge container exists
    const { container } = render(
      <TaskCard task={taskWithDate} onToggle={mockOnToggle} onDeleteClick={mockOnDeleteClick} onEdit={mockOnEdit} />
    );
  ...
  it('does not render description when description is empty', () => {
    ...
    const { container } = render(
      <TaskCard task={taskNoDesc} onToggle={mockOnToggle} onDeleteClick={mockOnDeleteClick} onEdit={mockOnEdit} />
    );
    const paragraphs = container.querySelectorAll('p');
    expect(paragraphs.length).toBe(0);
  });
  it('does not re-render if identical props are passed (React.memo)', () => {
    const spy = vi.spyOn(geo, 'calculateDistance');
    const taskWithLocation: Task = {
      ...baseTask,
      location: { lat: 40.4168, lng: -3.7038, address: 'Puerta del Sol' }
    };
    ...
    // Since it's memoized, it should not render again, so calculateDistance shouldn't be called again
    expect(spy).toHaveBeenCalledTimes(1);
