...
describe('TaskCard', () => {
  ...
  it('calls onDelete when delete button is clicked and user confirms via UI', () => {
    render(<TaskCard task={baseTask} onToggle={mockOnToggle} onDelete={mockOnDelete} onEdit={mockOnEdit} />);
    const deleteButton = screen.getByLabelText('Eliminar tarea');
    fireEvent.click(deleteButton);

    const confirmButton = screen.getByLabelText('Confirmar eliminar');
    fireEvent.click(confirmButton);

    expect(mockOnDelete).toHaveBeenCalledWith('1');
  });

  it('does not call onDelete if user cancels confirm via UI', () => {
    render(<TaskCard task={baseTask} onToggle={mockOnToggle} onDelete={mockOnDelete} onEdit={mockOnEdit} />);
    const deleteButton = screen.getByLabelText('Eliminar tarea');
    fireEvent.click(deleteButton);

    const cancelButton = screen.getByLabelText('Cancelar eliminar');
    fireEvent.click(cancelButton);

    expect(mockOnDelete).not.toHaveBeenCalled();
    expect(screen.queryByLabelText('Confirmar eliminar')).toBeNull();
  });
});
