...
import { DEFAULT_CENTER, DEFAULT_ICON, USER_ICON, COMPLETED_ICON, TASK_CIRCLE_OPTIONS, PREVIEW_CIRCLE_OPTIONS } from '../constants';
...
const LocateControl: React.FC<{ 
  ...
  const handleClick = (e: React.MouseEvent) => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        ...
        setLoading(false);
        if (err.code === 3) {
           setErrorMsg("GPS lento. Espera o muévete.");
        } else {
           setErrorMsg("Error al obtener ubicación.");
        }
        if (timeoutRef.current) clearTimeout(timeoutRef.current);
        timeoutRef.current = setTimeout(() => setErrorMsg(null), 4000);
      },
      GEOLOCATION_OPTIONS
    );
  };
  ...
  return (
    <div className="absolute bottom-6 right-4 z-[1000] flex flex-col items-end gap-2">
      {errorMsg && (
        <div className="bg-red-500 text-white text-xs px-3 py-1.5 rounded-lg shadow-md animate-in fade-in slide-in-from-bottom-2">
          {errorMsg}
        </div>
      )}
      <button
