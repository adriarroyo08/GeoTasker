...
describe('getCurrentPositionWithFallback', () => {
  ...
  it('should resolve with high accuracy location if successful', async () => {
  ...
  it('should immediately reject if permission is denied (code 1)', async () => {
    ...
    (global.navigator.geolocation.getCurrentPosition as any).mockImplementationOnce((_, error: any) => {
      error(mockError);
    });
  ...
  it('should fallback to low accuracy if high accuracy fails with timeout (code 3)', async () => {
    ...
    // Verify first call options
    expect((global.navigator.geolocation.getCurrentPosition as any).mock.calls[0][2].enableHighAccuracy).toBe(true);
    // Verify second call options
    expect((global.navigator.geolocation.getCurrentPosition as any).mock.calls[1][2].enableHighAccuracy).toBe(false);
  });
});
