...
export const parseTaskWithGemini = async (input: string): Promise<{ title: string; description: string; hasLocation: boolean; suggestedLocationName?: string; dueDate?: string }> => {
  try {
    ...
    const response = await ai.models.generateContent({
      ...
      Si el usuario menciona una fecha o fecha relativa (ej. "mañana"), calcúlala basada en la fecha actual del sistema (${currentDate}) y extráela en formato ISO 8601 en "dueDate". Si no, déjalo vacío.
